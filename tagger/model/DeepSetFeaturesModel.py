...
@JetModelFactory.register('DeepSetFeaturesModel')
class DeepSetFeaturesModel(DeepSetModel):
    """DeepSetFeaturesModel class
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        jet_features = tf.keras.layers.Input(shape=inputs_shape[1], name='features_input')
        ...
        # Concatenate with jet features
        main = tf.keras.layers.Concatenate(name='concat')([main, jet_features])
        ...
        self.jet_model = tf.keras.Model(inputs=[inputs, jet_features], outputs=[jet_id, pt_regress])
    ...
    def fit(
        ...
        Args:
            ...
            jet_features (npt.NDArray[np.float64]): Jet level features input
        ...
        inputs, jet_features = X_train
        self.history = self.jet_model.fit(
            {'model_input': X_train, 'features_input': jet_features},
        ...
        def firmware_convert(self, firmware_dir: str, build: bool = False):
            ...
            config['LayerName']['features_input']['Precision']['result'] = self.firmware_config['input_precision']
