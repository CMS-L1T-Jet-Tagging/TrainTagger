...
@JetModelFactory.register('DeepSetMaskingModel')
class DeepSetMaskingModel(DeepSetModel):
    """DeepSetMaskingModel class
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        mask = tf.keras.layers.Input(shape=inputs_shape[1], name='mask_input')
        ...
        main = tf.keras.layers.Multiply()([main, mask])
        main = agg(main)
        ...
        self.jet_model = tf.keras.Model(inputs=[inputs, mask], outputs=[jet_id, pt_regress])
    ...
    def fit(
        ...
        Args:
            X_train (npt.NDArray[np.float64]): X train dataset, containts inputs and mask
        ...
        self.history = self.jet_model.fit(
            {'model_input': inputs, 'mask_input': mask},
        ...
        def firmware_convert(self, firmware_dir: str, build: bool = False):
            ...
            config['LayerName']['mask_input']['Precision']['result'] = 'ap_uint<1>'
