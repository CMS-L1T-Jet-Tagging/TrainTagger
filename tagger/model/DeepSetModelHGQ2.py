...
from hgq.layers import QConv1D, QDense,QBatchNormalization, QGlobalAveragePooling1D
...
class DeepSetModelHGQ2(JetTagModel):
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        inputs = Input(shape=inputs_shape, name='model_input')
        main = QBatchNormalization(name='norm_input', iq_conf=None)(inputs)
        ...
        for iconv1d, depthconv1d in enumerate(self.model_config['conv1d_layers']):
            ...
            main = QConv1D(
                filters=depthconv1d, 
                parallelization_factor=depthconv1d, 
                kernel_size=1, 
