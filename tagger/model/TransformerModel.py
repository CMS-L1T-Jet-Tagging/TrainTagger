...
from keras.layers import BatchNormalization, Input, Activation, GlobalAveragePooling1D, GlobalMaxPooling1D, Dense, ReLU, LayerNormalization, MultiHeadAttention, Add
...
class TransformerModel(JetTagModel):
    ...
    def build_model(self, inputs_shape: tuple, outputs_shape: tuple):
        ...
        main = LayerNormalization(name='norm_input')(inputs)

        # Embedding
        for i, nodes in enumerate(self.model_config['embedding_layers']):
            main = Dense(nodes, activation='relu', name='emb_'+str(i+1), **self.common_args)(main)

        # Transformer blocks
        for i, (num_heads, ff_dim) in enumerate(self.model_config['transformer_layers']):
            d_model = self.model_config['embedding_layers'][-1]
            assert d_model % num_heads == 0, f"d_model ({d_model}) must be divisible by num_heads ({num_heads})"
            
            x = LayerNormalization(name='attn_mha_norm_'+str(i+1))(main)
            mha = MultiHeadAttention(num_heads, d_model // num_heads, name='attn_mha_'+str(i+1))(x, x)
            main = Add()([main, mha])

            x = LayerNormalization(name='attn_ff_norm_'+str(i+1))(main)
            ff = Dense(ff_dim, activation='relu', name='attn_dense_'+str(i+1), **self.common_args)(x)
            ff = Dense(d_model, name='attn_dense_out_'+str(i+1), **self.common_args)(ff)
            main = Add()([main, ff])
