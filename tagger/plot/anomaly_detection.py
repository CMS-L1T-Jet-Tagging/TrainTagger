...
def _get_styled_label(label: str) -> str:
    try:
        return style.CLASS_LABEL_STYLE[label]
    except KeyError:
        try:
            return style.PROCESS_LABEL_STYLE[label]
        except KeyError:
            return label
...
def plot_anomaly_score_per_class_hist(
    ...
    for cls, info in class_info.items():
        ...
        ax.hist(scores, bins=hist_edges, linewidth=style.LINEWIDTH, histtype='step', density=True, label=_get_styled_label(info['label']), color=info['color'])
    ...
    if log_scale:
        ax.set_xscale('log')
    ax.grid(True)
    ax.legend(fontsize=style.SMALL_SIZE-1, loc='upper right', title='Process', title_fontsize=style.SMALL_SIZE)
...
def plot_anomaly_score_vs_variable(
    ...
    for cls, info in class_info.items():
        ...
        class_variable = variable[label_mask]
        class_scores = test_scores[label_mask]

        var_bin_means = np.full(len(var_centers), np.nan)
        for i, (low, high) in enumerate(zip(var_edges[:-1], var_edges[1:])):
            bin_mask = ((class_variable >= low) & (class_variable < high))

            if np.any(bin_mask):
                var_bin_means[i] = np.mean(class_scores[bin_mask])
        ...
        ax.plot(
            ...
            label=_get_styled_label(info["label"]),
            color=info["color"],
        )

    # Add label for correlation
    corr = np.corrcoef(variable, test_scores)[0, 1]
    ax.text(0.97, 0.03, f'Correlation: {corr:.2f}', transform=ax.transAxes, fontsize=style.MEDIUM_SIZE + 2, ha='right', va='bottom')
    ...
    ax.grid(True)
    ax.legend(fontsize=style.SMALL_SIZE-1, loc='upper left', title='Process', title_fontsize=style.SMALL_SIZE)
    plt.savefig(output_file, bbox_inches="tight",)
    plt.close()
...
def plot_roc(
    ...
    """
    bkg_info = process_info[bkg_class]
    bkg_mask = event_classes == bkg_info['class']
    bkg_scores = event_scores[bkg_mask]
    ...
    for proc, info in process_info.items():
        ...
        sig_mask = event_classes == info['class']
        sig_scores = event_scores[sig_mask]
        ...
        ax.plot(tpr, bkg_rate, lw=style.LINEWIDTH, label=f"{_get_styled_label(info['label'])} (AUC={roc_auc:.2f})", color=info['color'])

    # Mark target operating point
    if rate_target is not None:
        ax.axhline(rate_target, color='red', linestyle='--', linewidth=style.LINEWIDTH, label=f'{rate_target:.0f} kHz')
    ...
    ax.grid(True)
    ax.legend(fontsize=style.SMALL_SIZE-1, loc='lower right', title='Process', title_fontsize=style.SMALL_SIZE)
    plt.savefig(output_file, bbox_inches="tight",)
    plt.close()
...
def plot_roc_vs_qcd(
    ...
    if weight_variable is not None:
        if weight_bins is None:
            raise ValueError("weight_bins must be provided when weight_variable is specified.")

        bkg_variable = weight_variable[bkg_mask]
        c_bkg, _ = np.histogram(bkg_variable, bins=weight_bins,)
    ...
    for proc, info in process_info.items():
        ...
        if weight_variable is not None:
            sig_variable = weight_variable[sig_mask]
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(tpr, fpr, lw=style.LINEWIDTH, label=f"{_get_styled_label(info['label'])} (AUC={roc_auc:.2f})", color=info['color'])
...
def plot_turn_on(
    ...
    for proc, info in process_info.items():
        ...
        proc_variable = turn_on_var[mask]
        proc_scores = event_scores[mask]

        total, _ = np.histogram(proc_variable, bins=bins)
        passed, _ = np.histogram(proc_variable[proc_scores > threshold], bins=bins)
        efficiency = np.divide(passed, total, out=np.zeros_like(passed, dtype=float), where=total > 0)

        ax.plot(bin_centers, efficiency, lw=style.LINEWIDTH, label=_get_styled_label(info['label']), marker='.', color=info['color'])
