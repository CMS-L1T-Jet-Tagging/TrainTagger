...
def loss_history(plot_dir, history):
    ...
    for metric in ["loss", "jet_id_output_loss", "pT_output_loss",
        ...
        hep.cms.label(llabel=style.CMSHEADER_LEFT,
                      rlabel=style.CMSHEADER_RIGHT, ax=ax, fontsize=style.CMSHEADER_SIZE)
        ax.plot(history.history[metric],
                label='Train Loss', linewidth=style.LINEWIDTH)
        ax.plot(history.history['val_'+metric],
                label='Validation Loss', linewidth=style.LINEWIDTH)
        ax.grid(True)
        # ax.set_ylabel('Loss')
...
def ROC_taus(y_pred, y_test, class_labels, plot_dir, signal_proc=None):
    ...
    jet_indices = [class_labels[key]
                   for key in ['b', 'charm', 'light', 'gluon']]
    ...
    for label, bkg_indices in [
        ...
        y_true, y_score = compute_roc_inputs(
            y_pred, y_test, tau_indices, bkg_indices)
...
def ROC_binary(y_pred, y_test, class_labels, plot_dir, class_pair, signal_proc=None):
    ...
    save_path = os.path.join(
        save_dir, f"ROC_{class_pair[0]}_vs_{class_pair[1]}")
    plt.savefig(f"{save_path}.pdf", bbox_inches='tight')
    plt.savefig(f"{save_path}.png", bbox_inches='tight')
    plt.close()
...
def ROC(y_pred, y_test, class_labels, plot_dir, ROC_dict):
    ...
    ax.legend([handles[idx] for idx in order], [labels[idx]
              for idx in order], loc='upper left', ncol=2, fontsize=style.SMALL_SIZE-3)
...
def confusion(y_pred, y_test, class_labels, plot_dir):
    ...
    labels = [style.CLASS_LABEL_STYLE[class_labels[i]]
              for i in range(y_test.shape[1])]

    # Create a plot of the confusion matrix
    fig, ax = plt.subplots(1, 1, figsize=style.FIGURE_SIZE)
    hep.cms.label(llabel=style.CMSHEADER_LEFT,
                  rlabel=style.CMSHEADER_RIGHT, fontsize=style.CMSHEADER_SIZE)
    ...
    plt.savefig(os.path.join(plot_dir, f"confusion_matrix.png"),
                bbox_inches='tight')
    plt.savefig(os.path.join(plot_dir, f"confusion_matrix.pdf"),
                bbox_inches='tight')
...
def response(class_labels, y_test, truth_pt_test, reco_pt_test, pt_ratio, plot_dir):
    ...
    pt_points = [np.mean((PT_BINS[i], PT_BINS[i + 1]))
                 for i in range(len(PT_BINS) - 1)]
    ...
    def plot_response(uncorrected_response, regressed_response, uncorrected_errors, regressed_errors, flavor, plot_name):
        ...
        hep.cms.label(llabel=style.CMSHEADER_LEFT,
                      rlabel=style.CMSHEADER_RIGHT, ax=ax, fontsize=style.CMSHEADER_SIZE)
        ax.errorbar(pt_points, uncorrected_response, yerr=uncorrected_errors, fmt='o',
                    label=f"Uncorrected - {style.CLASS_LABEL_STYLE[flavor]}", capsize=4, ms=8, elinewidth=3)
        ax.errorbar(pt_points, regressed_response, yerr=regressed_errors, fmt='o',
                    label=f"Regressed - {style.CLASS_LABEL_STYLE[flavor]}", capsize=4, ms=8, elinewidth=3)
    ...
    plot_response(uncorrected_response, regressed_response, uncorrected_errors,
                  regressed_errors, flavor='inclusive', plot_name="inclusive_response")
    ...
    for flavor in class_labels.keys():
        ...
        plot_response(uncorrected_response, regressed_response, uncorrected_errors,
                      regressed_errors, flavor=flavor, plot_name=f"{flavor}_response")
    ...
    for key in rms_selection.keys():
        ...
        plot_response(uncorrected_response, regressed_response, uncorrected_errors,
                      regressed_errors, flavor=key, plot_name=f"{key}_response")
...
def rms(class_labels, y_test, truth_pt_test, reco_pt_test, pt_ratio, plot_dir):
    ...
    def plot_rms(uncorrected_rms, regressed_rms, uncorrected_rms_err, regressed_rms_err, flavor, plot_name):
        ...
        hep.cms.label(llabel=style.CMSHEADER_LEFT,
                      rlabel=style.CMSHEADER_RIGHT, ax=ax, fontsize=style.CMSHEADER_SIZE)
        ax.errorbar(pt_points, uncorrected_rms, yerr=uncorrected_rms_err, fmt='o',
                    label=r"Uncorrected $\sigma$- {}".format(style.CLASS_LABEL_STYLE[flavor]), capsize=4, ms=8, elinewidth=3)
        ax.errorbar(pt_points, regressed_rms, yerr=regressed_rms_err, fmt='o',
                    label=r"Regressed $\sigma$ - {}".format(style.CLASS_LABEL_STYLE[flavor]), capsize=4, ms=8, elinewidth=3)
    ...
    uncorrected_rms, regressed_rms, uncorrected_rms_err, regressed_rms_err = get_rms(
        truth_pt_test, reco_pt_test, pt_ratio)
    plot_rms(uncorrected_rms, regressed_rms, uncorrected_rms_err,
             regressed_rms_err, flavor='inclusive', plot_name='inclusive')
    ...
    for flavor in class_labels.keys():
        ...
        uncorrected_rms, regressed_rms, uncorrected_rms_err, regressed_rms_err = get_rms(
            truth_pt_test[flavor_selection], reco_pt_test[flavor_selection], pt_ratio[flavor_selection])
        plot_rms(uncorrected_rms, regressed_rms, uncorrected_rms_err,
                 regressed_rms_err, flavor=flavor, plot_name=f"{flavor}_rms")
    ...
    for key in rms_selection.keys():
        ...
        uncorrected_rms, regressed_rms, uncorrected_rms_err, regressed_rms_err = get_rms(
            truth_pt_test[selection], reco_pt_test[selection], pt_ratio[selection])
        plot_rms(uncorrected_rms, regressed_rms, uncorrected_rms_err,
                 regressed_rms_err, flavor=key, plot_name=f"{key}_rms")
...
def shapPlot(shap_values, feature_names, class_names):
    ...
    for i, ind in enumerate(class_inds):
        ...
        ax.barh(y_pos, global_shap_values[feature_inds], 0.7, left=left_pos,
                align='center', label=label, color=colormap(class_inds[i]))
    ...
    ax.set_yticks(range(len(feature_order)), [
                  style.INPUT_FEATURE_STYLE[feature_names[i]] for i in feature_order], fontsize=30)
    ax.set_xlabel(
...
def efficiency(y_pred, y_test, reco_pt_test, class_labels, plot_dir):
    ...
    def plot_efficiency(df_wp_loose, df_wp_medium, df_wp_tight, plot_name):
        ...
        ax = df_wp_loose.plot.line(x="midpoints", y="wp_loose", yerr="err(wp_loose)",
                                   label="Loose (mistag = 50%)", color="blue", linestyle='-')
        df_wp_medium.plot.line(x="midpoints", y="wp_medium", yerr="err(wp_medium)",
                               label="Medium (eff = 20%)", ax=ax, color="orange", linestyle='-')
        df_wp_tight.plot.line(x="midpoints", y="wp_tight", yerr="err(wp_tight)",
                              label="Tight (eff = 10%)", ax=ax, color="green", linestyle='-')
    ...
    for key in eff_selection.keys():
        ...
        data_eff = ak.to_dataframe(ak.Array(
            {"truth": summed_y_true[selection], "pt": reco_pt_test[selection], "pred": summed_y_score[selection]}))
        ...
        h_wp_loose = histbook.Hist(histbook.split(
            "pt", pt_points), histbook.cut("wp_loose"))
        h_wp_medium = histbook.Hist(histbook.split(
            "pt", pt_points), histbook.cut("wp_medium"))
        h_wp_tight = histbook.Hist(histbook.split(
            "pt", pt_points), histbook.cut("wp_tight"))
        ...
        df_wp_loose["midpoints"] = [x[0].mid if isinstance(
            x[0], pandas.Interval) else np.nan for x in df_wp_loose.index]
        df_wp_medium["midpoints"] = [x[0].mid if isinstance(
            x[0], pandas.Interval) else np.nan for x in df_wp_medium.index]
        df_wp_tight["midpoints"] = [x[0].mid if isinstance(
            x[0], pandas.Interval) else np.nan for x in df_wp_tight.index]
...
def filter_process(test_data, process_dir):
    ...
    """
    train, test, class_labels = load_data(os.path.join(
        "signal_process_data", process_dir), percentage=100)[:3]
    ...
    process_data = np.unique(np.concatenate(
        (train[0], test[0]), axis=0), axis=0)
...
def basic(model, signal_dirs):
    ...
    truth_pt_test = np.load(
        f"{model.output_directory}/testing_data/truth_pt_test.npy")
    ...
    for i in range(-1, len(signal_dirs), 1):
        sample_plot_dir = os.path.join(
            model.output_directory, "plots/physics", f"binary_rocs_{signal_dirs[i]}")
        if i == -1:
            y_p, y_t = y_pred, y_test
            process_label = None
        else:
            signal_indices, sample_train, sample_test = filter_process(
                X_test, signal_dirs[i])
        ...
        for class_pair in class_pairs:
            binary_dir = os.path.join(
                sample_plot_dir, f"test_set") if i != -1 else plot_dir
            ROC_binary(y_p, y_t, model.class_labels,
                       binary_dir, class_pair, process_label)
            if i != -1:
                binary_dir = os.path.join(sample_plot_dir, "full_sample")
                ROC_binary(sample_preds, sample_labels, model.class_labels,
                           binary_dir, class_pair, process_label)

        # Add light vs b/charm/gluon combined plot
        binary_dir_test = os.path.join(
            sample_plot_dir, "test_set") if i != -1 else plot_dir
        ...
        if i != -1:
            ...
            ROC_jets(sample_preds, sample_labels, model.class_labels,
                     binary_dir_full, process_label)
            ROC_taus(sample_preds, sample_labels, model.class_labels,
                     binary_dir_full, process_label)
    ...
    response(model.class_labels, y_test, truth_pt_test,
             reco_pt_test, pt_ratio, plot_dir)

    # Plot the rms of the residuals vs pt
    rms(model.class_labels, y_test, truth_pt_test,
        reco_pt_test, pt_ratio, plot_dir)
