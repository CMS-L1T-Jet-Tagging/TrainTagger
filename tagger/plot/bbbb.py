...
def nn_bscore_sum(model, jet_nn_inputs, jet_pt, jet_eta, apply_light, n_jets=4, b_index=0, l_index=2, g_index=3):
    ...
    # Sum them together
    bscore_sum = sum(
        [x_vs_y(pred_score[:, b_index], pred_score[:, l_index] +
                pred_score[:, g_index], apply_light) for pred_score in nn_outputs]
    )
...
def pick_and_plot(rate_list, ht_list, nn_list, model_dir, apply_sel, apply_light, target_rate=14):
    ...
    interp_func = interp1d(target_rate_HT, target_rate_NN,
    ...
    working_point = {
        "HT": WPs_CMSSW['btag_l1_ht'], "NN": float(working_point_NN)}
    ...
    plt.savefig(
        f"{plot_dir}/bbbb_rate_{score_type}_{sel_type}.pdf", bbox_inches='tight')
    plt.savefig(
        f"{plot_dir}/bbbb_rate_{score_type}_{sel_type}.png", bbox_inches='tight')
...
def derive_bbbb_WPs(model, minbias_path, apply_sel, apply_light, target_rate=14, n_entries=100, tree='outnano/Jets'):
    ...
    jet_nn_inputs = jet_nn_inputs[default_selection(
    ...
    sel_ht = ak.sum(jet_pt, axis=1)[
        default_selection(jet_pt, jet_eta, apply_sel)]
    ...
    for ht in ht_edges[:-1]:
        for NN in NN_edges[:-1]:
            ...
            # Calculate the rate
            rate = RateHist[{"ht": slice(
                ht*1j, None, sum)}][{"nn": slice(NN*1.0j, None, sum)}]/n_events
            rate_list.append(rate*MINBIAS_RATE)
    ...
    pick_and_plot(rate_list, ht_list, nn_list, model.output_directory,
                  apply_sel, apply_light, target_rate=target_rate)
    ...
    derive_HT_WP(RateHist, ht_edges, n_events,
                 model.output_directory, target_rate=target_rate)
...
def load_bbbb_WPs(model_dir, apply_sel, apply_light):
    ...
    WP_path = os.path.join(
        model_dir, f"plots/physics/bbbb/working_point_{score_type}_{sel_type}.json")
    HT_WP_path = os.path.join(
        model_dir, f"plots/physics/bbbb/ht_working_point.json")
...
def bbbb_eff(model, signal_path, apply_sel, apply_light, n_entries=100000, tree='outnano/Jets'):
    ...
    btag_wp, btag_ht_wp, ht_only_wp = load_bbbb_WPs(
        model.output_directory, apply_sel, apply_light)
    ...
    if raw_gen_mHH is not None:
        event_id, grouped_gen_arrays = group_id_values(
            raw_event_id, raw_gen_mHH, raw_jet_genpt, num_elements=0)
        all_event_gen_mHH = ak.firsts(grouped_gen_arrays[0])
    else:
        # If genHH_mass doesn't exist, only group event_id and genpt
        event_id, grouped_gen_arrays = group_id_values(
            raw_event_id, raw_jet_genpt, num_elements=0)
    ...
    # Only sum up the first four
    cmsssw_bscore_sum = ak.sum(cmssw_bscore[:, :4], axis=1)
    ...
    cmssw_selection = (jet_ht > cmssw_btag_ht) & (
        cmsssw_bscore_sum > cmssw_btag)
    cmssw_efficiency = np.round(ak.sum(cmssw_selection) / n_events, 2)
    model_selection = (jet_ht > btag_ht_wp) & (
        model_bscore_sum > btag_wp) & default_selection(jet_pt, jet_eta, apply_sel)
    ...
    plot_path = os.path.join(
        model.output_directory, f"plots/physics/bbbb/HH_eff_HT_{score_type}_{sel_type}")
    ...
    # Save second plot
    ht_compare_path = os.path.join(
        model.output_directory, f"plots/physics/bbbb/HH_eff_HT_vs_HTonly_{score_type}_{sel_type}")
...
def bbbb_eff_mHH(model_dir,
    ...
    # Plot ht distribution in the background
    counts, bin_edges = np.histogram(
        np.clip(event_gen_mHH, 0, 800), bins=np.arange(0, 800, 40))
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    bin_width = bin_edges[1] - bin_edges[0]
    normalized_counts = counts / np.sum(counts)
    ...
    btag_wp, btag_ht_wp, ht_only_wp = load_bbbb_WPs(
        model_dir, apply_sel, apply_light)
    ...
    hep.histplot((normalized_counts, bin_edges), ax=ax,
                 histtype='step', color='grey', label=r"$mHH^{gen}$")
    ax.errorbar(model_x, model_y, yerr=model_err, c=style.color_cycle[1], fmt='o', linewidth=3,
                label=r'Multiclass @ 14 kHz, {}={} (L1 $HT$ > {} GeV, $\sum$ 4b > {})'.format(eff_str, model_efficiency, btag_ht_wp, round(btag_wp, 2)))
    ...
    sel_type = "sel" if apply_sel else "all"
    ht_compare_path = os.path.join(
        model_dir, f"plots/physics/bbbb/HH_eff_mHH_{score_type}_{sel_type}")
    plt.savefig(f'{ht_compare_path}.pdf', bbox_inches='tight')
    plt.savefig(f'{ht_compare_path}.png', bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-s', '--sample', default='/eos/cms/store/cmst3/group/l1tr/sewuchte/l1teg/fp_jettuples_090125_addGenH/GluGluHHTo4B_PU200.root', help='Signal sample for HH->bbbb')
    ...
    parser.add_argument('--eff', action='store_true',
                        help='plot efficiency for HH->4b')
    ...
    if args.deriveWPs:
        derive_bbbb_WPs(model, args.minbias, True, True,
                        n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        derive_bbbb_WPs(model, args.minbias, True, False,
                        n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        derive_bbbb_WPs(model, args.minbias, False, True,
                        n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        derive_bbbb_WPs(model, args.minbias, False, False,
                        n_entries=args.n_entries, tree=args.tree)
    elif args.eff:
        bbbb_eff(model, args.sample, True, True,
                 n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbbb_eff(model, args.sample, True, False,
                 n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbbb_eff(model, args.sample, False, True,
                 n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbbb_eff(model, args.sample, False, False,
                 n_entries=args.n_entries, tree=args.tree)
