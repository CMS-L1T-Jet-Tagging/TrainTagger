...
def nn_bscore_sum(model, jet_nn_inputs, n_jets=4, b_index = 1):
    ...
    #Get the nn outputs
    nn_outputs = [model.predict([nn_input, nn_input[:, :2, :]]) for nn_input in btag_inputs]
...
def derive_bbbb_WPs(model_dir, minbias_path, target_rate=14, n_entries=100, tree='outnano/Jets'):
    ...
    nn_outputs = [model.predict([np.asarray(jet_nn_inputs[:, i]), np.asarray(jet_nn_inputs[:, i])[:,:2,:]]) for i in range(0,4)]
