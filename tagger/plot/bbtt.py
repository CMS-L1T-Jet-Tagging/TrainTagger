...
from tagger.plot.common import MINBIAS_RATE, find_rate, plot_ratio, get_bar_patch_data
...
def default_selection(jet_pt, jet_eta, indices, apply_sel):
    if apply_sel == "tau":
        ...
        mask = (jet_pt[rows, indices] > 7) & (
            np.abs(jet_eta[rows, indices]) < 2.4)
...
def max_tau_sum(taup_preds, taum_preds):
    ...
    """
    taup_argsort, taum_argsort = np.argsort(taup_preds, axis=1)[
        :, ::-1][:, :2], np.argsort(taum_preds, axis=1)[:, ::-1][:, :2]
    rows = np.arange(len(taup_preds)).reshape((-1, 1))
    alt_scores = taup_preds[rows, taup_argsort] + \
        taum_preds[rows, taum_argsort][:, ::-1]
    tau_alt_idxs = np.stack((taup_argsort[:, 0], taum_argsort[:, 1]), axis=-1)
    tau_alt2_idxs = np.stack((taup_argsort[:, 1], taum_argsort[:, 0]), axis=-1)
    tau_alt_idxs[ak.argmax(alt_scores, axis=1) ==
                 1] = tau_alt2_idxs[ak.argmax(alt_scores, axis=1) == 1]
    tau_scores1 = taup_preds[rows, taup_argsort[:, 0].reshape(
        -1, 1)] + taum_preds[rows, taum_argsort[:, 0].reshape(-1, 1)]
    tau_scores1 = tau_scores1.flatten()
    tau_scores1[taup_argsort[:, 0] == taum_argsort[:, 0]] = ak.max(
        alt_scores, axis=1)[taup_argsort[:, 0] == taum_argsort[:, 0]]
    tau_idxs = np.stack((taup_argsort[:, 0], taum_argsort[:, 0]), axis=-1)
    tau_idxs[taup_argsort[:, 0] == taum_argsort[:, 0]
             ] = tau_alt_idxs[taup_argsort[:, 0] == taum_argsort[:, 0]]
...
def nn_score_sums(model, jet_nn_inputs, n_jets=4):
    ...
    # vs light preds
    taup_vs_qg = np.transpose([x_vs_y(
        pred_score[:, tp_idx], pred_score[:, l_idx] + pred_score[:, g_idx]) for pred_score in nn_outputs])
    taum_vs_qg = np.transpose([x_vs_y(
        pred_score[:, tm_idx], pred_score[:, l_idx] + pred_score[:, g_idx]) for pred_score in nn_outputs])
    b_vs_qg = np.transpose([x_vs_y(pred_score[:, b_idx], pred_score[:,
                           l_idx] + pred_score[:, g_idx]) for pred_score in nn_outputs])

    # raw preds
    taup_preds = np.transpose([pred_score[:, tp_idx]
                              for pred_score in nn_outputs])
    taum_preds = np.transpose([pred_score[:, tm_idx]
                              for pred_score in nn_outputs])
...
def pick_and_plot(rate_list, signal_eff, ht_list, bb_list, tt_list, ht, score_type, apply_sel, model_dir, n_entries, rate, tree):
    ...
    target_rate_idx = find_rate(
        rate_list, target_rate=rate, RateRange=RateRange)
...
def make_predictions(model, data_path, n_entries, tree='outnano/Jets', njets=4):
    ...
    event_id, grouped_arrays = group_id_values(
        raw_event_id, raw_jet_pt, raw_jet_eta, raw_inputs, num_elements=4)
...
def derive_HT_WP(RateHist, ht_edges, n_events, model_dir, target_rate, RateRange=0.85):
    ...
    for ht in ht_edges[:-1]:
        ...
        # Calculate the rate
        rate = RateHist[{"ht": slice(ht*1j, None, sum)}
                        ][{"nn": slice(0.0j, None, sum)}]/n_events
...
def derive_bbtt_WPs(model, minbias_path, ht_cut, apply_sel, signal_path, n_entries=100, tree='outnano/Jets'):
    ...
    bscore_sums = [b_sum[def_sel]
                   for b_sum, def_sel in zip(bscore_sums, def_sels)]
    tscore_sums = [t_sum[def_sel]
                   for t_sum, def_sel in zip(tscore_sums, def_sels)]

    jet_ht = ak.sum(jet_pt, axis=1)

    # Define the histograms (pT edge and NN Score edge)
    # Make sure to capture everything
    ht_edges = list(np.arange(150, 500, 1)) + [10000]
    ...
    s_bscore_sums = [b_sum[def_sel]
                     for b_sum, def_sel in zip(s_bscore_sums, s_def_sels)]
    s_tscore_sums = [t_sum[def_sel]
                     for t_sum, def_sel in zip(s_tscore_sums, s_def_sels)]

    # Rate Hists for rate derivation
    RateHistRaw = Hist(hist.axis.Variable(ht_edges, name="ht", label="ht"),
                       hist.axis.Variable(
                           NN_edges, name="nn_bb", label="nn_bb"),
    ...
    RateHistRaw.fill(ht=jet_ht[def_sels[0]],
                     nn_bb=bscore_sums[0], nn_tt=tscore_sums[0])
    RateHistQG.fill(ht=jet_ht[def_sels[1]],
                    nn_bb=bscore_sums[1], nn_tt=tscore_sums[1])
    ...
    SHistRaw.fill(ht=signal_ht[s_def_sels[0]],
                  nn_bb=s_bscore_sums[0], nn_tt=s_tscore_sums[0])
    SHistQG.fill(ht=signal_ht[s_def_sels[1]],
                 nn_bb=s_bscore_sums[1], nn_tt=s_tscore_sums[1])
    ...
    def parallel_in_parallel(tt, bb):
        # Calculate the rate
        counts_raw = RateHistRaw[{"ht": slice(ht_cut*1j, None, sum)}][{"nn_bb": slice(
            bb*1.0j, None, sum)}][{"nn_tt": slice(tt*1.0j, None, sum)}]
        rate_raw = (counts_raw / n_events)*MINBIAS_RATE

        counts_qg = RateHistQG[{"ht": slice(ht_cut*1j, None, sum)}][{"nn_bb": slice(
            bb*1.0j, None, sum)}][{"nn_tt": slice(tt*1.0j, None, sum)}]
        rate_qg = (counts_qg / n_events)*MINBIAS_RATE

        # check if the rate is in the range, skip if not
        if pick_rates(rate_raw) and pick_rates(rate_qg):
            return

        # get signal efficiencies
        counts_signal_raw = SHistRaw[{"ht": slice(ht_cut*1j, None, sum)}][{
            "nn_bb": slice(bb*1.0j, None, sum)}][{"nn_tt": slice(tt*1.0j, None, sum)}]
        eff_signal_raw = counts_signal_raw / s_n_events

        counts_signal_qg = SHistQG[{"ht": slice(ht_cut*1j, None, sum)}][{"nn_bb": slice(
            bb*1.0j, None, sum)}][{"nn_tt": slice(tt*1.0j, None, sum)}]
    ...
    start = time.time()
    parallel_out = Parallel(n_jobs=3)(
        delayed(parallel_in_parallel_wrapper)(bb) for bb in tqdm(NN_edges[:-1]))
    ...
    ht_list, bb_list, tt_list = np_out[:, 4].tolist(
    ), np_out[:, 5].tolist(), np_out[:, 6].tolist()

    # Pick target rate and plot it
    pick_and_plot(rate_list_raw, eff_list_raw, ht_list, bb_list, tt_list,
                  ht_cut, 'raw', apply_sel, model.output_directory, n_entries, rate, tree)
    gc.collect()
    pick_and_plot(rate_list_raw, eff_list_raw, ht_list, bb_list, tt_list,
                  ht_cut, 'raw', apply_sel, model.output_directory, n_entries, 14, tree)
    gc.collect()
    pick_and_plot(rate_list_qg, eff_list_qg, ht_list, bb_list, tt_list,
                  ht_cut, 'qg', apply_sel, model.output_directory, n_entries, rate, tree)
    gc.collect()
    pick_and_plot(rate_list_qg, eff_list_qg, ht_list, bb_list, tt_list,
                  ht_cut, 'qg', apply_sel, model.output_directory, n_entries, 14, tree)
    gc.collect()
...
def bbtt_eff_HT(model, signal_path, score_type, apply_sel, n_entries=100000, tree='outnano/Jets'):
    ...
    WP_path = os.path.join(
        model.output_directory, f"plots/physics/bbtt/bbtt_fixed_wp_{score_type}_{apply_sel}_{rate}.json")
    WP_path_14 = os.path.join(
        model.output_directory, f"plots/physics/bbtt/bbtt_fixed_wp_{score_type}_{apply_sel}_14.json")
    HT_path = os.path.join(model.output_directory,
                           "plots/physics/bbtt/ht_working_point.json")
    ...
    raw_inputs = extract_nn_inputs(
        signal, model.input_vars, n_entries=n_entries)
    ...
    if score_type == 'raw':
        ...
        default_sel = default_selection(
            jet_pt, jet_eta, tau_indices[0], apply_sel)
    else:
        model_bscore_sum = model_bscore_sums[1]
        model_tscore_sum = model_tscore_sums[1]
        default_sel = default_selection(
            jet_pt, jet_eta, tau_indices[1], apply_sel)

    model_selection = (jet_ht > ht_wp) & (model_bscore_sum > btag_wp) & (
        model_tscore_sum > ttag_wp) & default_sel
    model_efficiency = np.round(np.sum(model_selection) / n_events, 2)
    model_selection_14 = (jet_ht > ht_wp_14) & (model_bscore_sum > btag_wp_14) & (
        model_tscore_sum > ttag_wp_14) & default_sel
    ...
    counts, bin_edges = np.histogram(
        np.clip(jet_genht, 0, 800), bins=np.arange(0, 800, 40))
    ...
    hep.histplot((normalized_counts, bin_edges), ax=ax,
                 histtype='step', color='grey', label=r"$HT^{gen}$")
    ax.errorbar(baseline_x, baseline_y, yerr=baseline_err, c=style.color_cycle[0], fmt='o', linewidth=3,
                label=r'bb$\tau \tau$ seed@ {} kHz, {}={} (L1 $HT$ > {} GeV, {} > {} GeV)'.format(rate, eff_str, baseline_efficiency, 220, tau_str, 34))
    ax.errorbar(model_x, model_y, yerr=model_err, c=style.color_cycle[1], fmt='o', linewidth=3, label=r'Multiclass @ {} kHz, {}={} (L1 $HT$ > {} GeV, $\sum$ $\tau\tau$ > {}, $\sum$ bb > {})'.format(
    ...
    ax.set_ylabel(
        r"$\epsilon$(HH $\to$ bb$\tau \tau$ trigger rate at {} kHz)".format(rate))
    plt.legend(loc='upper left')

    # Save plot
    plot_path = os.path.join(
        model.output_directory, f"plots/physics/bbtt/HHbbtt_eff_bbtt_seed_{score_type}_{apply_sel}")
    ...
    hep.cms.label(llabel=style.CMSHEADER_LEFT,
                  rlabel=style.CMSHEADER_RIGHT, ax=ax2, fontsize=style.MEDIUM_SIZE-2)
    hep.histplot((normalized_counts, bin_edges), ax=ax2,
                 histtype='step', color='grey', label=r"$HT^{gen}$")
    ax2.errorbar(model_x_14, model_y_14, yerr=model_err, c=style.color_cycle[1], fmt='o', linewidth=3, label=r'Multiclass @ {} kHz, {}={} (L1 $HT$ > {} GeV, $\sum$ $\tau\tau$ > {}, $\sum$ bb > {})'.format(
    ...
    ax2.errorbar(ht_only_x, ht_only_y, yerr=ht_only_err, c=style.color_cycle[2], fmt='o', linewidth=3,
                 label=r'HT-only + QuadJets @ {} kHz, {}={} (L1 $HT$ > {} GeV)'.format(14, eff_str, ht_only_efficiency, ht_only_wp))
    ...
    # Save plot
    plot_path = os.path.join(
        model.output_directory, f"plots/physics/bbtt/HHbbtt_eff_HT_only_{score_type}_{apply_sel}")
    plt.savefig(f'{plot_path}.pdf', bbox_inches='tight')
    plt.savefig(f'{plot_path}.png', bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-s', '--signal', default='/eos/cms/store/cmst3/group/l1tr/sewuchte/l1teg/fp_jettuples_090125_addGenH/GluGluHHTo2B2Tau_PU200.root', help='Signal sample for HH->bbtt')
    parser.add_argument('--minbias', default='/eos/cms/store/cmst3/group/l1tr/sewuchte/l1teg/fp_jettuples_090125/MinBias_PU200.root',
                        help='Minbias sample for deriving rates')

    # Different modes
    parser.add_argument('--deriveRate', action='store_true',
                        help='derive the rate for the bbtt seed')
    parser.add_argument('--deriveWPs', action='store_true',
                        help='derive the working points for b-tagging')
    parser.add_argument('--eff', action='store_true',
                        help='plot efficiency for HH->bbtt')

    parser.add_argument('--tree', default='outnano/Jets',
                        help='Tree within the ntuple containing the jets')

    # Other controls
    parser.add_argument('-n', '--n_entries', type=int, default=1000,
    ...
    if args.deriveRate:
        derive_rate(model, args.minbias,
                    n_entries=args.n_entries, tree=args.tree)
    if args.deriveWPs:
        derive_bbtt_WPs(model, args.minbias, 220, 'tau',
                        args.signal, n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        derive_bbtt_WPs(model, args.minbias, 220, 'all',
                        args.signal, n_entries=args.n_entries, tree=args.tree)
    elif args.eff:
        bbtt_eff_HT(model, args.signal, 'raw', 'tau',
                    n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbtt_eff_HT(model, args.signal, 'qg', 'tau',
                    n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbtt_eff_HT(model, args.signal, 'raw', 'all',
                    n_entries=args.n_entries, tree=args.tree)
        gc.collect()
        bbtt_eff_HT(model, args.signal, 'qg', 'all',
                    n_entries=args.n_entries, tree=args.tree)
