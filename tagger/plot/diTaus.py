...
def pick_and_plot_ditau(rate_list, pt_list, nn_list, model_dir, target_rate=28, RateRange=1.0):
    ...
    # + 0.02 #WP looks a bit too loose for taus using interpolation so just a quick hack
    working_point_NN = interp_func(WPs_CMSSW['tau_l1_pt'])

    # Export the working point
    working_point = {
        "PT": WPs_CMSSW['tau_l1_pt'], "NN": float(working_point_NN)}
...
def derive_diTaus_WPs(model, minbias_path, target_rate=28, n_entries=100, tree='jetntuple/Jets'):
    ...
    input1, input2 = np.asarray(jet_nn_inputs[:, 0][cuts]), np.asarray(
        jet_nn_inputs[:, 1][cuts])
    ...
    for pt in pT_edges[:-1]:
        for NN in NN_edges[:-1]:
            ...
            rate = RateHist[{"pt": slice(
                pt*1j, None, sum)}][{"nn": slice(NN*1.0j, None, sum)}]/n_events
    ...
    pick_and_plot_ditau(rate_list, pt_list, nn_list,
...
def plot_bkg_rate_ditau(model, minbias_path, n_entries=500000, tree='jetntuple/Jets'):
    ...
    #
    nn_inputs = np.asarray(extract_nn_inputs(
        minbias, model.input_vars, n_entries=n_entries))
    ...
    # Emulator tau score
    cmssw_tau = extract_array(minbias, 'jet_tauscore', n_entries)[
        eta_selection]
    ...
    WP_path = os.path.join(model.output_directory,
                           "plots/physics/tautau/working_point.json")
    ...
    jet_pt_cmssw = extract_array(minbias, 'jet_taupt', n_entries)[
        eta_selection][cmssw_tau > WPs_CMSSW["tau"]]
    ...
    ax.plot(pt_cuts, minbias_rate_no_nn,
            c=style.color_cycle[2], label=r'No ID/$p_T$ correction', linewidth=style.LINEWIDTH)
    ax.plot(pt_cuts, minbias_rate_cmssw,
            c=style.color_cycle[0], label=r'CMSSW PuppiTau Emulator', linewidth=style.LINEWIDTH)
    ax.plot(pt_cuts, minbias_rate_model,
            c=style.color_cycle[1], label=r'SeedCone Tau', linewidth=style.LINEWIDTH)
    ...
    fig.savefig(os.path.join(plot_dir, "tautau_BkgRate.pdf"),
                bbox_inches='tight')
    fig.savefig(os.path.join(plot_dir, "tautau_BkgRate.png"),
                bbox_inches='tight')
...
def eff_ditau(model, signal_path, eta_region='barrel', tree='jetntuple/Jets', n_entries=10000):
    ...
    pT_egdes = [0, 10, 15, 20, 25, 30, 35, 40, 45,
                50, 55, 60, 70, 80, 100, 125, 150, 175, 200]
    ...
    nn_tauscore_raw = pred_score[:, model.class_labels['taup'],
    ...
    tau_nume_seedcone = tau_deno & (
        np.abs(gen_dr_raw) < 0.4) & (l1_pt_raw > model_pt_WP)
    tau_nume_nn = tau_deno & (np.abs(gen_dr_raw) < 0.4) & (
        nn_taupt_raw > model_pt_WP) & (nn_tauscore_raw > model_NN_WP)
    tau_nume_cmssw = tau_deno & (np.abs(gen_dr_raw) < 0.4) & (
        jet_taupt_raw > WPs_CMSSW['tau_l1_pt']) & (jet_tauscore_raw > WPs_CMSSW['tau'])
    ...
    ax.errorbar(sc_x, sc_y, yerr=sc_err, fmt='o',
                c=style.color_cycle[2], markersize=style.LINEWIDTH, linewidth=2, label=r'SeededCone PuppiJet Efficiency Limit')
    ax.errorbar(cmssw_x, cmssw_y, yerr=cmssw_err, fmt='o',
                c=style.color_cycle[0], markersize=style.LINEWIDTH, linewidth=2, label=r'Tau CMSSW Emulator @ 28kHz')
    ax.errorbar(nn_x, nn_y, yerr=nn_err, fmt='o',
                c=style.color_cycle[1], markersize=style.LINEWIDTH, linewidth=2, label=r'SeededCone Tau Tagger @ 28kHz')

    # Plot a horizontal dashed line at y=1
    ax.axhline(1, xmin=0, xmax=150, linestyle='dashed',
...
if __name__ == "__main__":
    ...
    # Other controls
    parser.add_argument('-n', '--n_entries', type=int, default=500000,
    ...
    if args.deriveWPs:
        derive_diTaus_WPs(model, args.minbias,
                          n_entries=args.n_entries, tree=args.tree)
    elif args.BkgRate:
        plot_bkg_rate_ditau(model, args.minbias,
                            n_entries=args.n_entries, tree=args.tree)
    elif args.eff:
        eff_ditau(model, args.vbf_sample, n_entries=args.n_entries,
                  eta_region='barrel', tree=args.tree)
        eff_ditau(model, args.vbf_sample, n_entries=args.n_entries,
                  eta_region='endcap', tree=args.tree)
