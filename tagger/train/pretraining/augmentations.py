...
class AugmentationLayer(keras.layers.Layer):
    def __init__(
            self,
            augmentations: list[keras.layers.Layer] = [FlatMaskingLayer(isFilled_idx=15, pt_index=0, masking_probability=0.2)],
        ):
        super().__init__()
        self.augment = tf.keras.Sequential(augmentations)
